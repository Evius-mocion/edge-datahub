...
export class EdgeService {
  ...
  async registerAttendee(dto: AttendeeRegisterPayload): Promise<any> {
    ...
    const attendeExiste = await this.attendeeRepo.findOne({
      where: { eventId, email },
    });
  ...
  async logExperiencePlay(
    dto: ExperiencePlayPayload,
  ): Promise<{ message: string; play: any }> {
    const {
      eventExperienceId,
      attendeeId,
      play_timestamp,
    ...
    if (!eventExperience) {
      throw new NotFoundException(
        `Event Experience with ID ${eventExperienceId} not found.`,
      );
    }
    ...
    if (existingPlay) {
      ...
      return {
        message: 'Experience play updated successfully',
        play: savedPlay,
      };
    }
  ...
  async redeemPoints(
    ...
    const [totalPoints, _redemptionPoints] = await this.get_total_points_user(
      attendee.id,
      attendee.eventId,
    );
    ...
    if (pointsRedeemed > totalPoints) {
      throw new BadRequestException(
        `Cannot redeem ${pointsRedeemed} points — you only have ${totalPoints} points`,
      );
    }
  ...
  async attendeeStatus(
    ...
    const whereClause =
      [{ id: attendeeId }, { code }, { email }].find(
        (condition) => Object.values(condition)[0],
      ) || null;

    if (!whereClause) {
      throw new BadRequestException(
        'You must provide attendeeId, code, or email.',
      );
    }
    ...
    const [totalPoints, redemptionPoints] = await this.get_total_points_user(
      attendee.id,
      attendee.eventId,
    );
    ...
    const status = {
      ...
      availablePoints: totalPoints - (redemptionPoints || 0),
    };
  ...
  async get_total_points_user(attendee_id: string, event_id: string) {
    const { totalPoints } = (await this.experiencePlayData
      .createQueryBuilder('play')
      .select(
        'SUM(COALESCE(play.score, 0) + COALESCE(play.bonusScore, 0))',
        'totalPoints',
      )
    ...
    const { totalRedeemed } = (await this.pointsRedemptionRepo
